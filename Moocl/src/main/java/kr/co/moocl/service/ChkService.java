...
public class ChkService {
	...
	public void chk(List<String[]> wordList, int idx, List<String[]> conList, List<String> finalList) {
		if (idx + 1 == wordList.size()) {
			...
			if (wordList.get(idx)[1].equals("NNG") || wordList.get(idx)[1].equals("NNP")
					|| wordList.get(idx)[1].equals("NNB")) {
				if (idx + 1 < wordList.size() && (wordList.get(idx + 1)[1].equals("NNG")
						|| wordList.get(idx + 1)[1].equals("NNP") || wordList.get(idx + 1)[1].equals("NNB"))) {
					conList.add(wordList.get(idx + 1));
					// finalList.add(conList.stream().collect(Collectors.toList()));
					finalList.add(wordMakingFunction(conList.stream().collect(Collectors.toList())));

					chk(wordList, idx + 1, conList, finalList);
				} else if (idx + 1 < wordList.size() && wordList.get(idx + 1)[1].equals("JKG")) {

					if (idx + 2 < wordList.size() && (wordList.get(idx + 2)[1].equals("NNG")
							|| wordList.get(idx + 2)[1].equals("NNP") || wordList.get(idx + 2)[1].equals("NNB"))) {
						conList.add(wordList.get(idx + 1));
				...
				} else if (idx + 1 < wordList.size() && (wordList.get(idx + 1)[1].equals("VA"))) {
					conList.add(wordList.get(idx + 1));
					finalList.add(wordMakingFunction(conList.stream().collect(Collectors.toList())));
					chk(wordList, idx + 1, conList, finalList);
				}
				...
				// }
						...
						&& (wordList.get(idx + 2)[1].equals("NNG") || wordList.get(idx + 2)[1].equals("NNP")
								|| wordList.get(idx + 2)[1].equals("NNB"))) {
					conList.add(wordList.get(idx + 1));
					conList.add(wordList.get(idx + 2));
					finalList.add(wordMakingFunction(conList.stream().collect(Collectors.toList())));
					chk(wordList, idx + 2, conList, finalList);
				...
				if (idx + 3 < wordList.size() && wordList.get(idx + 1)[1].equals("XSA")
						&& wordList.get(idx + 2)[1].equals("ETM") && (wordList.get(idx + 3)[1].equals("NNG")
								|| wordList.get(idx + 3)[1].equals("NNP") || wordList.get(idx + 3)[1].equals("NNB"))) {
					conList.add(wordList.get(idx + 1));
					conList.add(wordList.get(idx + 2));
			...
			} else if (wordList.get(idx)[1].equals("SN")) {
				if (idx + 1 < wordList.size() && (wordList.get(idx + 1)[1].equals("NNG")
						|| wordList.get(idx + 1)[1].equals("NNP") || wordList.get(idx + 1)[1].equals("NNB"))) {
					conList.add(wordList.get(idx + 1));
					// finalList.add(conList.stream().collect(Collectors.toList()));
	...
	public void pickup(List<String[]> wordList, List<String> finalList) {
		for (String[] word : wordList) {
			if (word[1].equals("NNG") || word[1].equals("NNP") || word[1].equals("NA") || word[1].equals("NF")
					|| word[1].equals("NV") || word[1].equals("IC")) {
	...
	public List<Character> fml(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			...
			int cho = ac / CHOSUNG;
			int jung = (ac - (CHOSUNG * cho)) / JUNGSUNG;
			int jong = (ac - (CHOSUNG * cho) - (JUNGSUNG * jung));
	...
	public char makeChar(char chosung, char jungsung, char jongsung) {
		...
		int CHOSUNG = 588;
		int JUNGSUNG = 28;
		char[] CHOSUNG_LIST = { 'ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ',
				'ㅍ', 'ㅎ' };
		char[] JUNGSUNG_LIST = { 'ㅏ', 'ㅐ', 'ㅑ', 'ㅒ', 'ㅓ', 'ㅔ', 'ㅕ', 'ㅖ', 'ㅗ', 'ㅘ', 'ㅙ', 'ㅚ', 'ㅛ', 'ㅜ', 'ㅝ', 'ㅞ', 'ㅟ',
				'ㅠ', 'ㅡ', 'ㅢ', 'ㅣ' };
		char[] JONGSUNG_LIST = { ' ', 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄹ', 'ㄺ', 'ㄻ', 'ㄼ', 'ㄽ', 'ㄾ', 'ㄿ', 'ㅀ', 'ㅁ',
	...
	public String wordMakingFunction(List<String[]> wordList) {
		String finalWord = "";
		int i = 0;
