<template lang="html">
    <v-layout justify-center align-center>
      <v-avatar xs3 size="300" class="pb-5 mx-5 mb-5"><img src="../assets/cgv_icon.png" style=""></v-avatar>
      <v-flex xs4 class="pb-5 mb-5">
        <v-text-field label="Nickname" v-model="nickname" :rules="nicknameRules" :counter="4" required></v-text-field>
        <v-form v-model="valid" ref="form" lazy-validation>
          <v-text-field label="New Password" v-model="password" :rules="passwordRules" :append-icon="e1 ? 'visibility' : 'visibility_off'" :append-icon-cb="() => (e1 = !e1)" :type="e1 ? 'password' : 'text'":counter="8" required></v-text-field>
          ...
          <v-btn @click="submit" :disabled="!valid"> 완료 </v-btn>
        </v-form>
      </v-flex>
    </v-layout>
</template>
